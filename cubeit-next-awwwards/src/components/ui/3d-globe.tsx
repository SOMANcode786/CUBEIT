    ...
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={cn(
              "group relative cursor-pointer overflow-visible rounded-full bg-neutral-900 shadow-lg transition-transform duration-200",
              hovered && "scale-125 shadow-xl ring-1 ring-white/50",
            )}
            style={{
              width: `${Math.max(7, Math.round(markerSize * 135))}px`,
              height: `${Math.max(7, Math.round(markerSize * 135))}px`,
            }}
          ...
          >
            <span className="block h-full w-full overflow-hidden rounded-full">
              {/* eslint-disable-next-line @next/next/no-img-element -- rendered inside a Three.js Html portal */}
              <img src={marker.src} alt={marker.label || "Marker"} className="h-full w-full object-cover" draggable={false} />
            </span>
            {marker.label ? <span className="pointer-events-none absolute left-1/2 top-[12px] w-max -translate-x-1/2 rounded-full border border-white/15 bg-black/70 px-2 py-1 text-[6px] font-semibold text-white opacity-0 backdrop-blur-md transition-opacity group-hover:opacity-100">{marker.label}</span> : null}
          </div>
        </Html>
      </group>
    </group>
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }: SceneProps) {
  ...
  return (
    <>
      ...
      <hemisphereLight args={["#dce8ff", "#160f26", config.ambientIntensity * 1.2]} />
      <directionalLight
      ...
      />
      <pointLight
        position={[-config.radius * 2.6, config.radius * 2.2, config.radius * 3.4]}
        intensity={config.pointLightIntensity * 1.15}
        color="#b998ff"
        distance={config.radius * 12}
      />
