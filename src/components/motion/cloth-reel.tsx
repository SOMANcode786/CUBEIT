...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    // The bottom-left corner stays pinned while the cloth whips outward.
    // Displacement grows with distance from that held point, producing an
    // asymmetric whip motion rather than a uniform plane.
...
export default function ClothReel({ originSelector, targetSelector, stackSelector, src, poster }: ClothReelProps) {
  ...
  useEffect(() => {
    ...
    camera.position.z = 3;
    geometry = new THREE.PlaneGeometry(1, 1, 38, 28);
    ...
    const render = (now: number) => {
      ...
      const borderRadius = `${mix(18, 26, view.sizeProgress)}px`;
      fallback.style.borderRadius = borderRadius;
      fallback.style.clipPath = `inset(0 round ${borderRadius})`;
